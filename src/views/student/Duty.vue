  ...
  <section>
    <el-row style='margin: 10px 10px 10px 10px;'>
      <el-breadcrumb separator-class='el-icon-arrow-right'>
        <el-breadcrumb-item :to='{path: "/duty"}'>学生管理</el-breadcrumb-item>
        <el-breadcrumb-item>值日管理</el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    ...
    <el-card style='margin: 20px 1%;width: 100%'>
      <el-col :span='24' class='toolbar' style='padding-bottom: 0px'>
        <el-form :inline='true' :model='filters'>
          ...
          <el-form-item>
            <el-button type='primary' v-on:click='getDutys'>查询</el-button>
          </el-form-item>
          <el-form-item>
      ...
      <el-table :data="dutys" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
        ...
        <el-table-column prop='begDate' label='开始日期' width='120' sortable></el-table-column>
        ...
        <el-table-column prop='roster' label='值班名单' width='120' sortable></el-table-column>
    ...
    <el-dialog title='编辑' :visible.sync='editFormVisible' :close-on-click-modal='false'>
      <el-form :model='editForm' label-width='80px' :rules='editFormRules' ref='editForm'>
        ...
        <el-form-item label='值班名单' prop='roster'>
          <el-input v-model='editForm.roster' auto-complete='off'></el-input>
        </el-form-item>
    ...
    <el-dialog title='新增' :visible.sync='addFormVisible' :close-on-click-modal='false'>
      <el-form :model='addForm' label-width='80px' :rules='addFormRules' ref='addForm'>
        ...
        <el-form-item label="星期周数" prop='weekNo'>
           <el-select v-model="addForm.weekNo" placeholder="请选择">
              <el-option
        ...
        <el-form-item label='值班名单' prop='roster'>
          <el-input v-model='addForm.roster' auto-complete='off'></el-input>
        </el-form-item>
        <el-form-item label='备注' prop='memo'>
...
export default {
  ...
  data () {
    return {
      ...
      addFormRules: {
        ...
        begDate: [{ required: true, message: '请输入开始日期', trigger: 'blur' }],
        endDate: [{ required: true, message: '请输入结束日期', trigger: 'blur' }],
        weekNo: [{ required: true, message: '请输入星期周数', trigger: 'blur' }],
        roster: [{ required: true, message: '请输入值班名单', trigger: 'blur' }]
      },
      // 新增界面数据
      addForm: {
        classOwner: '',
  ...
  methods: {
    ...
    getDutys () {
      ...
      var _this = this
      /*
      this.$axios.post('/student/duty/select', {
          page: this.page,
          name: this.filters.name
        }).then(res => {
          if (res.data.code === 200) {
            _this.dutys = res.data
          }
        }).catch(failResponse => {})
        */
      this.$axios.get('/student/duty/select').then(res => {
    ...
    handleDel: function (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.listLoading = true
        // NProgress.start()
        let para = { id: row.dutyId }
        // eslint-disable-next-line no-undef
        // 提交数据到后台
        this.$axios.post('/student/duty/delete', {
          id: para.id
        }).then(res => {
          this.listLoading = false
          // NProgress.done()
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/student/duty/update', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/student/duty/insert', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
    ...
    batchRemove: function () {
      var ids = this.sels.map(item => item.dutyId).toString()
