  ...
  <section>
    <el-row style='margin: 10px 10px 10px 10px;'>
      <el-breadcrumb separator-class='el-icon-arrow-right'>
        <el-breadcrumb-item :to='{path: "/room"}'>学生管理</el-breadcrumb-item>
    ...
    <el-card style='margin: 20px 1%;width: 100%'>
      ...
      <el-table :data="rooms" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
        ...
        <el-table-column prop='roomName' label='寝室名称' width='120' sortable></el-table-column>
    ...
    <el-dialog title='编辑' :visible.sync='editFormVisible' :close-on-click-modal='false'>
      <el-form :model='editForm' label-width='80px' :rules='editFormRules' ref='editForm'>
        <el-form-item label='寝室名称' prop='roomName'>
...
export default {
  ...
  data () {
    return {
      ...
      addLoading: false,
      addFormRules: {
        roomName: [{ required: true, message: '请输入寝室名称', trigger: 'blur' }]
      },
      // 新增界面数据
      addForm: {
        roomName: '',
  ...
  methods: {
    ...
    getRooms () {
      ...
      var _this = this
      /*
      this.$axios.post('/student/room/select', {
          page: this.page,
        ...
        */
      this.$axios.get('/student/room/select').then(res => {
    ...
    handleDel: function (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.listLoading = true
        // NProgress.start()
        let para = { id: row.roomId }
        // eslint-disable-next-line no-undef
        // 提交数据到后台
        this.$axios.post('/student/room/delete', {
          id: para.id
        }).then(res => {
          this.listLoading = false
          // NProgress.done()
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/student/room/update', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/student/room/insert', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
    ...
    batchRemove: function () {
      var ids = this.sels.map(item => item.roomId).toString()
