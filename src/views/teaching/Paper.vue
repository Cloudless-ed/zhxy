  ...
  <section>
    <el-row style="margin: 10px 10px 10px 10px;">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to='{path: "/paper"}'>教学管理</el-breadcrumb-item>
    ...
    <el-card style="margin: 20px 1%;width: 100%">
      ...
      >
        ...
        <el-table-column prop="paperName" label="试卷名称" width="120" sortable></el-table-column>
        <el-table-column prop="memo" label="备注" width="120" sortable></el-table-column>
    ...
    <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
      <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
        ...
        <el-form-item label="试卷名称" prop="paperName">
          <el-input v-model="addForm.paperName" auto-complete="off" maxlength="20"></el-input>
        </el-form-item>
        <el-form-item label="试卷说明" prop="paperDesc">
          <el-input type="textarea" v-model="addForm.paperDesc" auto-complete="off" maxlength="100" show-word-limit></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="memo">
...
export default {
  ...
  data() {
    // /^[a-z0-9A-Z\u4e00-\u9fa5]+$/gi
    var paperName = (rule, value, callback) => {
    ...
    return {
      ...
      addFormRules: {
        paperName: [
          ...
          { validator: paperName, trigger: "blur" }
        ],
        course: [
  ...
  methods: {
    ...
    getPapers() {
      ...
      var _this = this;
      /*
      this.$axios.post('/teaching/paper/select', {
          page: this.page,
          name: this.filters.name
        }).then(res => {
          if (res.data.code === 200) {
            _this.papers = res.data
          }
        }).catch(failResponse => {})
        */
      this.$axios.post("/api/teaching/paper/select",{
        pageId:'1',
        limit:'1',
        searchKey:'1'
      }).then(res => {
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          // NProgress.start()
          let para = { id: row.paperId };
          // eslint-disable-next-line no-undef
          // 提交数据到后台
          this.$axios
            .post("*/api/teaching/paper/delete", {
              // id: para.id
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // 提交数据到后台
            this.$axios
              .post("/api/teaching/paper/update", {
                // id: para.id,
                // cover: para.cover,
                // title: para.title,
                // author: para.author,
                ...
                paperCode:para.paperCode,
                paperName:para.paperName,
                paperDesc:para.paperDesc,
                memo:para.memo
              })
              .then(res => {
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // 提交数据到后台
            this.$axios
              .post("/api/teaching/paper/insert", {
                // id: para.id,
                // cover: para.cover,
                // title: para.title,
                // author: para.author,
