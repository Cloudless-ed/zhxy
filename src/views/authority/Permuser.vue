  ...
  <section>
    <el-row style='margin: 10px 10px 10px 10px;'>
      <el-breadcrumb separator-class='el-icon-arrow-right'>
        <el-breadcrumb-item :to='{path: "/permuser"}'>权限管理</el-breadcrumb-item>
    ...
    <el-card style='margin: 20px 1%;width: 100%'>
      <el-col :span='24' class='toolbar' style='padding-bottom: 0px'>
        <el-form :inline='true' :model='filters'>
          ...
          <el-form-item>
            <el-button type='primary' v-on:click='getPermusers'>查询</el-button>
          </el-form-item>
          <el-form-item>
      ...
      <el-table :data="permusers" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
...
export default {
  ...
  methods: {
    ...
    getPermusers () {
      ...
      var _this = this
      /*
      this.$axios.post('/authority/permuser/select', {
          page: this.page,
          name: this.filters.name
        }).then(res => {
          if (res.data.code === 200) {
            _this.permusers = res.data
          }
        }).catch(failResponse => {})
        */
      this.$axios.get('/authority/permuser/select').then(res => {
    ...
    handleDel: function (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        this.listLoading = true
        // NProgress.start()
        let para = { id: row.permuserId }
        // eslint-disable-next-line no-undef
        // 提交数据到后台
        this.$axios.post('/authority/permuser/delete', {
          id: para.id
        }).then(res => {
          this.listLoading = false
          // NProgress.done()
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/authority/permuser/update', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/authority/permuser/insert', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
    ...
    batchRemove: function () {
      var ids = this.sels.map(item => item.permuserId).toString()
