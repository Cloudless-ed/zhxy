  ...
  <section>
    <el-row style='margin: 10px 10px 10px 10px;'>
      <el-breadcrumb separator-class='el-icon-arrow-right'>
        <el-breadcrumb-item :to='{path: "/unit"}'>组织机构</el-breadcrumb-item>
        ...
        <el-table-column type='expand'>
          <template slot-scope='props'>
            <el-form label-position='left' inline>
              <el-form-item>
                <span>{{ props.row.unitDesc }}</span>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
        <el-table-column prop='party.partyCode' label='单位编码' width='120' sortable></el-table-column>
        <el-table-column prop='party.partyName' label='单位名称' width='120' sortable></el-table-column>
        <el-table-column prop='principal' label='单位负责人' width='120' sortable></el-table-column>
    ...
    <el-dialog title='编辑' :visible.sync='editFormVisible' :close-on-click-modal='false'>
      <el-form :model='editForm' label-width='80px' :rules='editFormRules' ref='editForm'>
        <el-form-item label='单位编码' prop='partyCode'>
        ...
        <el-form-item label='上级单位' style="display:none;" prop='parentId'>
        ...
        <el-form-item label='单位简介' prop='deptDesc'>
...
export default {
  ...
  methods: {
    ...
    getUnits () {
      ...
      var _this = this
      /*
      this.$axios.post('/authority/unit/select', {
          page: this.page,
        ...
        */
      this.$axios.get('/api/authority/unit/select').then(res => {
    ...
    handleDel: function (index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        // 提交数据到后台
        this.$axios.post('/authority/unit/delete', {
          id: para.id
        }).then(res => {
          this.listLoading = false
          // NProgress.done()
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/authority/unit/update', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            // eslint-disable-next-line no-undef
            // 提交数据到后台
            this.$axios.post('/authority/unit/insert', {
              id: para.id,
              cover: para.cover,
              title: para.title,
              author: para.author,
    ...
    batchRemove: function () {
      var ids = this.sels.map(item => item.unitId).toString()
