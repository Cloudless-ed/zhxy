  ...
  <section>
    <el-row style="margin: 10px 10px 10px 10px;">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to='{path: "/lecturer"}'>教务管理</el-breadcrumb-item>
    ...
    <el-card style="margin: 20px 1%;width: 100%">
      <el-col :span="24" class="toolbar" style="padding-bottom: 0px">
        <el-form :inline="true" :model="filters">
          ...
          <el-form-item>
            <el-button type="primary" v-on:click="getLecturers">查询</el-button>
          </el-form-item>
          <el-form-item>
      ...
      >
        ...
        <el-table-column prop="limitNum" label="学生上限" width="120" sortable></el-table-column>
    ...
    <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false">
      <el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
        ...
        <el-form-item label="学生上限" prop="limitNum">
          <el-input-number v-model="editForm.limitNum" :min="0" :max="999"></el-input-number>
        </el-form-item>
    ...
    <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
      <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
        ...
        <el-form-item label="学生上限" prop="limitNum">
          <el-input-number v-model="addForm.limitNum" :min="0" :max="999"></el-input-number>
        </el-form-item>
        <el-form-item label="备注" prop="memo">
...
export default {
  ...
  methods: {
    ...
    getLecturers() {
      ...
      var _this = this;
      /*
      this.$axios.post('/educational/lecturer/select', {
          page: this.page,
          name: this.filters.name
        }).then(res => {
          if (res.data.code === 200) {
            _this.lecturers = res.data
          }
        }).catch(failResponse => {})
        */
      this.$axios.post("/api/educational/lecturer/select",{
        pageId:'1',
        limit:'1',
        searchKey:'1'
      }).then(res => {
    ...
    handleDel: function(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          this.listLoading = true;
          // NProgress.start()
          let para = { id: row.lecturerId };
          // eslint-disable-next-line no-undef
          // 提交数据到后台
          this.$axios
            .post("/api/educational/lecturer/delete", {
              // id: para.id
      ...
      this.$refs.editForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // 提交数据到后台
            this.$axios
              .post("/api/educational/lecturer/update", {
                // id: para.id,
                // cover: para.cover,
                // title: para.title,
                // author: para.author,
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            // 提交数据到后台
            this.$axios
              .post("/api/educational/lecturer/insert", {
                // id: para.id,
                // cover: para.cover,
                // title: para.title,
                // author: para.author,
    ...
    batchRemove: function() {
      var ids = this.sels.map(item => item.lecturerId).toString();
